/* Adapted from tree-sitter-grammars/tree-sitter-hcl.
...
static unsigned serialize(Scanner *scanner, char *buf) {
    ...
    for (uint32_t i = 0; i < scanner->context_stack.len; i++) {
        ...
        if (size + sizeof(ContextType) + sizeof(uint32_t) +
        ...
        memcpy(
            &buf[size],
            context->heredoc_identifier.data,
            context->heredoc_identifier.len
        );
...
static bool scan(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (valid_symbols[TEMPLATE_INTERPOLATION_START] &&
        valid_symbols[TEMPLATE_LITERAL_CHUNK] && !in_interpolation_context(scanner) &&
        lexer->lookahead == '$') {
        advance(lexer);
        if (lexer->lookahead == '{') {
            Context ctx;
    ...
    if (valid_symbols[TEMPLATE_DIRECTIVE_START] &&
        valid_symbols[TEMPLATE_LITERAL_CHUNK] && !in_directive_context(scanner) &&
        lexer->lookahead == '%') {
        advance(lexer);
        if (lexer->lookahead == '{') {
            Context ctx;
    ...
    if (valid_symbols[TEMPLATE_DIRECTIVE_END] && in_directive_context(scanner) &&
        lexer->lookahead == '}') {
        VEC_POP(scanner->context_stack);
...
void tree_sitter_hcl_external_scanner_deserialize(
    void *payload,
    const char *buffer,
    unsigned length
) {
