  ...
  rules: {
    ...
    run_instruction: ($) =>
      seq(
        ...
        choice($.command_json_string_array, $.shell_command),
    ...
    stopsignal_instruction: ($) =>
      seq(alias(/[sS][tT][oO][pP][sS][iI][gG][nN][aA][lL]/, "STOPSIGNAL"), $._stopsignal_value),
    ...
    shell_instruction: ($) =>
      seq(alias(/[sS][hH][eE][lL][lL]/, "SHELL"), $.command_json_string_array),

    maintainer_instruction: () =>
      seq(alias(/[mM][aA][iI][nN][tT][aA][iI][nN][eE][rR]/, "MAINTAINER"), /.*/),

    cross_build_instruction: () =>
      seq(alias(/[cC][rR][oO][sS][sS]_[bB][uU][iI][lL][dD][a-zA-Z_]*/, "CROSS_BUILD"), /.*/),
    ...
    _env_key: ($) => alias(/[a-zA-Z_][a-zA-Z0-9_]*/, $.unquoted_string),
    ...
    image_spec: ($) =>
      seq(
        ...
        seq(field("tag", optional($.image_tag)), field("digest", optional($.image_digest)))
      ),
    ...
    param: ($) =>
      seq(
        "--",
        field("name", alias(token.immediate(/[a-z][-a-z]*/), $.param_name)),
        token.immediate("="),
        field(
          "value",
          alias(
    ...
    mount_param: ($) =>
      seq(
        "--",
        field("name", alias(token.immediate("mount"), $.param_name)),
        token.immediate("="),
        field("value", $.mount_param_value)
      ),
    ...
    mount_param_param: ($) =>
      choice(
        seq(
          field("key", alias(token.immediate("type"), $.mount_param_key)),
          token.immediate("="),
          field("value", alias($._mount_param_text_value, $.mount_param_enum_value))
        ),
        seq(
          field("key", alias(token.immediate("sharing"), $.mount_param_key)),
          token.immediate("="),
          field("value", alias($._mount_param_text_value, $.mount_param_enum_value))
        ),
        seq(
          field(
            "key",
            alias(
              token.immediate(choice("target", "source", "src", "dst", "destination", "from", "id")),
              $.mount_param_key
            )
          ),
          token.immediate("="),
          field("value", alias($._mount_param_text_value, $.mount_param_string_value))
        ),
        seq(
          field("key", alias(token.immediate("env"), $.mount_param_key)),
          token.immediate("="),
          field("value", alias($._mount_param_text_value, $.mount_param_identifier_value))
        ),
        seq(
          field(
            "key",
            alias(token.immediate(choice("uid", "gid", "mode")), $.mount_param_key)
          ),
          token.immediate("="),
          field("value", alias(token.immediate(/[0-9]+/), $.mount_param_numeric_value))
        ),
        seq(
          field(
            "key",
            alias(token.immediate(choice("readonly", "required")), $.mount_param_key)
          ),
          token.immediate("="),
          field(
            "value",
            alias(token.immediate(choice("true", "false")), $.mount_param_boolean_value)
          )
        ),
        seq(
          field("key", alias(token.immediate(/[^\s=,]+/), $.mount_param_key)),
          token.immediate("="),
          field("value", alias($._mount_param_text_value, $.mount_param_value_part))
        ),
        alias(token.immediate(choice("readonly", "required")), $.mount_param_flag),
        alias(token.immediate(/[^\s=,]+/), $.mount_param_flag)
      ),

    _mount_param_text_value: ($) =>
      repeat1(choice(token.immediate(/[^\s=,\$]+/), $._immediate_expansion)),
    ...
    shell_command: ($) =>
      seq(
        $.shell_fragment,
        repeat(seq(alias($.required_line_continuation, $.line_continuation), $.shell_fragment))
      ),
    ...
    json_string_array: ($) =>
      seq("[", optional(seq($.json_string, repeat(seq(",", $.json_string)))), "]"),
    ...
    command_json_string_array: ($) =>
      seq(
        ...
        optional(
          seq(
            alias($.json_string, $.json_string_command),
            repeat(seq(",", $.json_string))
          )
        ),
        "]"
      ),
    ...
    json_string: ($) =>
      seq(
        '"',
        repeat(
          choice(token.immediate(/[^"\\]+/), alias($.json_escape_sequence, $.escape_sequence))
        ),
        '"'
      ),

    json_escape_sequence: () => token.immediate(/\\(?:["\\/bfnrt]|u[0-9A-Fa-f]{4})/),
    ...
    unquoted_string: ($) =>
      repeat1(
        choice(token.immediate(/[^\s\n\"'\\\$]+/), token.immediate("\\ "), $._immediate_expansion)
      ),
