...
module.exports = grammar({
  ...
  rules: {
    ...
    _group_expression: ($) => choice($._definition, immediate_child($._group_expression)),

    _named_node_expression: ($) =>
      choice($._definition, $.negated_field, immediate_child($._named_node_expression)),
    ...
    grouping: ($) =>
      seq("(", repeat($._group_expression), ")", quantifier($), captures($)),
    ...
    predicate: ($) =>
      seq(
        ...
        field(
          "name",
          seq("#", $._immediate_identifier, field("type", $.predicate_type)),
        ),
...
function immediate_child(expression) {
  return prec.left(PREC.IMMEDIATE_CHILD, seq(expression, ".", expression));
}
